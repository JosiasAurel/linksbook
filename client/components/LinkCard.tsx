...
const LinkCard: React.FC<LinkCardProps> = ({
  ...
  // modal state
  const [confimDeleteModal, setConfirmDeleteModal] =
  ...
  const [removeLink, { data: data_, loading: loading_, error: error_ }] =
    useMutation(REMOVE_LINK_FROM_COLLECTION);

  function removeBookmarkFromFolder(): void {
    toast.promise(
      removeLink({
        variables: { linkId: id, collectionId: folderId },
        refetchQueries: [{ query: FETCH_ALL }],
      })
        ...
        {tags.map((tag) => {
          return (
            <Tag
              name={tag}
              key={tag}
              searchByTag={(name) => tagSearchHandler(name)}
            />
          );
        })}
      </div>
        ...
        <Modal.Content>
          ...
          <div className={styles.confirmDeleteModalStyles}>
            ...
            {inFolder ? (
              <>
                <Button
                  onClick={() => removeBookmarkFromFolder()}
                  color="error"
                >
