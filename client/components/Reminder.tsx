...
function UpdateReminderComponent({ getUpdatedData, finishAction, reminder, oldData }): JSX.Element {

    const [updateReminder, { data, loading, error }] = useMutation(UPDATE_REMINDER);

    // set reminder form
    const [remindDate, setRemindDate] = React.useState(new Date(oldData.remindDate));
    ...
    const [recipients, setRecipients] = React.useState<Array<string>>(oldData.recipients);
    function recipientsHandler(v: any): void {
        setRecipients(v.split(" "));
    }

    function handleUpdateReminder(): void {
        toast.promise(updateReminder({ variables: { reminderId: reminder, remindDate: prepareUTCRemindDate(), recipients: recipients.join(" ").trim().split(" ") }, refetchQueries: [{ query: FETCH_ALL }] }), {
    ...
    return (
        <div>
            ...
            <div>
                ...
                <Spacer />
                <Button onClick={e_ => handleUpdateReminder()}>
                    Save Reminder
                </Button>
            </div>
        </div>
...
function ReminderTooltipBody({ data_, date, recipients, reminder, getUpdatedData, linkId }): JSX.Element {
    ...
    return (
        <>
            ...
            <div style={{ display: "flex", flexDirection: "row" }}>
                <Button ghost type="success" onClick={_ => setUpdateModal(!updateModal)}>
                    Update
                </Button>
                <Spacer />
...
const Reminder: React.FC<ReminderProps> = ({ data, recipients, date, id, getUpdatedData, linkId }): JSX.Element => {
    return (
        <Tooltip text={<ReminderTooltipBody data_={data} linkId={linkId} getUpdatedData={d => getUpdatedData(d)} reminder={id} recipients={recipients} date={date} />} placement="left">
            <Card shadow>
                <Badge>
                    {recipients.length}
                </Badge>
